...
- We could probably go straight from the ocean model to physical evaluation.
- For the hybrid or AI model things are a little trickier. 
  - Massage the outputs into a compatible format - we enforce that for the evaluation to work they must comply with restart format and output format. We currently only support NEMO. But long term we'd like to extend this. 
- Use this translator toolbox
- This may seem weird but we have designed these two components in the context of hybrid modelling. This is our 'glue' that enables us to restart models easily with AI emulated states.  Which gives us an extra form of evaluation
:::
